...
### 1. Chinese Text Pipeline · 可追溯的中文数据加工
...
[项目](https://github.com/kimzclandi/chinese-text-pipeline) · [运行](https://github.com/kimzclandi/chinese-text-pipeline/blob/main/docs/REPRODUCE.md) · [结果](https://github.com/kimzclandi/chinese-text-pipeline/blob/main/reports/RESULTS.md)

### 2. Detection Data Selection · 固定预算下的检测数据选择

在 BDD100K 小样本上比较随机与定向选样，仅微调预训练检测器的 ROI 预测头。提供三种子、匹配训练步数的对照、失败切片及排序机制诊断；当前未证明定向选择稳定优于随机。Ray 扩展仅在单机验证。

[项目与运行](https://github.com/kimzclandi/detection-data-selection) · [训练对照](https://github.com/kimzclandi/detection-data-selection/blob/main/docs/FAILURE_V2_REPORT.md) · [机制诊断](https://github.com/kimzclandi/detection-data-selection/blob/main/docs/DIAGNOSIS_V3_REPORT.md)

### 3. VLM Image Ablation · 视觉输入干预与失败分析

固定 SmolVLM，对合成场景执行原图、空白图和错配图干预，保存 90 题 × 三种输入、共 270 次真实生成与配对切片结果。观察到的视觉贡献主要来自空间题；这是推理评测，没有模型训练收益。历史 metadata 规则演示与真实推理分开记录。

[项目](https://github.com/kimzclandi/vlm-image-ablation) · [运行](https://github.com/kimzclandi/vlm-image-ablation/blob/main/docs/RUNNING.md) · [干预结果](https://github.com/kimzclandi/vlm-image-ablation/blob/main/docs/GROUNDING_V3_REPORT.md)

### 4. Extractive QA · 数据覆盖、训练与量化对照
...
[项目](https://github.com/kimzclandi/extractive-qa) · [运行](https://github.com/kimzclandi/extractive-qa/blob/main/docs/QUALITY_STUDY_RELEASE.md) · [质量对照与限制](https://github.com/kimzclandi/extractive-qa/blob/main/reports/quality-study-20260920/RESULTS.md) · [外部评测](https://github.com/kimzclandi/extractive-qa/blob/main/docs/EXTERNAL_DRCD.md)

### 5. Shard Runner · 可恢复的数据分片处理
...
[项目与运行](https://github.com/kimzclandi/shard-runner) · [架构与语义](https://github.com/kimzclandi/shard-runner/blob/main/docs/ARCHITECTURE.md) · [故障实验](https://github.com/kimzclandi/shard-runner/blob/main/docs/RESULTS.md)

## 工程补充与配套研究

| 项目 | 独立问题与验证边界 |
|---|---|
| [Intent Distillation](https://github.com/kimzclandi/intent-distillation) | 8类意图的教师一致性筛选、三种子响应蒸馏与FP16/Q8/Q4对照；筛选未优于随机、绝对准确率偏低。保存数值导出故障、有限性门禁与修正证据。 |
...
| [Label Review](https://github.com/kimzclandi/label-review) | 缓存检测预测上的合成污染排序与定位诊断；真实标签效用仍在研究中，59 图保留池未评分。 |
