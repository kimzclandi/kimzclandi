...
| [Domain QA Lab](https://github.com/kimzclandi/domain-qa-lab) | 模型实验主项目；训练、数据质量对照、量化与冻结评估 | 使用参考标签；CMRC正向主比较未在DRCD外部96题复现；无业务部署 | `scripts/verify_quality_release.py`与`scripts/verify_external_drcd.py`；完整训练入口已同机重跑 |
...
| [Distill Quant Intent Lab](https://github.com/kimzclandi/distill-quant-intent-lab) | 独立压缩实验；教师双提示、响应蒸馏、受检CPU导出与MLX量化 | 8类小样本；筛选无优势，准确率不足以部署；数值故障及修复均保留 | 无模型运行`python scripts/verify.py`，训练与量化另需MPS/MLX |
...
- 新增Distill Quant Intent Lab作为小规模独立研究，未将压缩率或单机速度数字写成部署能力。
