...
主要项目依次为 ChineseTextProcessingAndRetrieval、ObjectDetectionDataSelection、VLMImageDependenceEvaluation、SmallModelQAFinetuningAndQuantization，分别对应数据资产与可靠性、检测数据选择、视觉输入评测、训练与量化对照。LeaseBasedShardScheduling 为可靠性工程补充；AgentGate 与 bearing-fault-diagnosis 为合作项目。更新介绍时保留仓库名称、URL 与 Git 历史。
