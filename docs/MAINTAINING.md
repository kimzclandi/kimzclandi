...
主要项目依次为 ChineseTextProcessingAndRetrieval、ObjectDetectionDataSelection、VLMImageDependenceEvaluation、SmallModelQAFinetuningAndQuantization，分别对应数据资产与可靠性、检测数据选择、视觉输入评测、训练与量化对照。AgentGate 为合作工程补充；LeaseBasedShardScheduling、RoadSequenceSelectionUnderBudget 与 DetectionLabelReviewAndLocalization 为配套研究。更新介绍时保留仓库名称、URL 与 Git 历史。
